...
def parse_pdf(file_path: str):
    ...
    with logfire.span("📄 Document AI Parsing", filename=file_path):
        try:
            ...
            location = settings.GCP_DOC_AI_LOCATION
            processor_id = settings.GCP_DOC_AI_PROCESSOR_ID

            if not project_id:
                raise ValueError("PROJECT_ID is not configured in settings.")
            if not location:
                raise ValueError("GCP_DOC_AI_LOCATION is not configured in settings.")
            if not processor_id:
                raise ValueError("GCP_DOC_AI_PROCESSOR_ID is not configured in settings.")
